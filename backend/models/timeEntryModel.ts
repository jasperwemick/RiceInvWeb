...
import { RiceEventDoc } from "./riceEventsModel";
...
timeEntrySchema.post('findOneAndUpdate', async function (doc : Document, next) {

    try {

        // Find the events involving the player whos time was just updated
        const thisTimeDoc = await this.model.findOne(this.getQuery())
        const relevantEvents = await doc.model('RiceEvent').find<RiceEventDoc>({participants: { $all: [thisTimeDoc.profileId]}, finished: false}).exec()
